...
func (r room) Less(i int, j int) bool {
	...
	return r.count[a] > r.count[b]
}

func (r room) Swap(i int, j int) {
	r.res[i], r.res[j] = r.res[j], r.res[i]
}

func (r *room) toMap(name string) {
	r.count = map[byte]int{}

	for _, char := range []byte(name) {
		r.count[char]++
	}

	r.res = make([]byte, 0, len(r.count))

	for k := range r.count {
		r.res = append(r.res, k)
	}
}

func (r *room) shiftName() {
	var name = []byte(r.name)
	var shift = r.id - r.id/26*26
...
func (r room) check() bool {
	return string(r.res[:5]) == r.checksum
}
...
func solution(data string) int {
	for _, line := range convert(data) {
		...
		if line.name == "northpole object storage" {
			return line.id
		}
	}
...
func convert(data string) []room {
	var lines []room

	for _, l := range strings.Split(data, "\n") {
		var line room

		nameEnd := strings.LastIndexByte(l, '-')
		notName := strings.Split(l[nameEnd:], "[")

		line.toMap(strings.ReplaceAll(l[:nameEnd], "-", ""))
		line.name = (strings.ReplaceAll(l[:nameEnd], "-", " "))
		line.checksum = strings.ReplaceAll(notName[1], "]", "")
		line.id = utils.ToInt(strings.ReplaceAll(notName[0], "-", ""))
...
const in = `aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]
jsehsyafy-vqw-ljs0000000000afafy-866[nymla]
