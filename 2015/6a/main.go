...
func (op operation) switchArea(lights map[point]bool) {
	for x := op.start.x; x <= op.end.x; x++ {
		for y := op.start.y; y <= op.end.y; y++ {
			switch op.typ {
			case on:
				lights[point{x, y}] = true
			case off:
				lights[point{x, y}] = false
			case toggle:
				lights[point{x, y}] = !lights[point{x, y}]
			}
		}
	}
}
...
func solution(data string) int {
	var sum int
	var ops = convert(data)
	var lights = map[point]bool{}

	for _, op := range ops {
		op.switchArea(lights)
	}
...
func convert(data string) []operation {
	var res []operation
...
func parse(line string) operation {
	...
	parts := strings.Split(strings.TrimPrefix(strings.ReplaceAll(line, ",", " "), "turn "), " ")

	return operation{
		typ:   opType(parts[0]),
		start: point{utils.ToInt(parts[1]), utils.ToInt(parts[2])},
		end:   point{utils.ToInt(parts[4]), utils.ToInt(parts[5])},
	}
}

const in = `turn on 887,9 through 959,629
